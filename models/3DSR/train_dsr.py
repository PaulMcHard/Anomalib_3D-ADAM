...
def evaluate_model(model, sub_res_model_lo, sub_res_model_hi, embedder_hi, embedder_lo, model_decode, decoder_seg, visualizer, obj_name, n_iter, img_min, img_max, mvtec_path):
    ...
    display_images = torch.zeros((16,1,img_dim,img_dim)).cuda()
    display_rgb_images = torch.zeros((16,3,img_dim,img_dim)).cuda()
    display_gt_images = torch.zeros((16,1,img_dim,img_dim)).cuda()
    display_gt_images_rgb = torch.zeros((16,3,img_dim,img_dim)).cuda()
    display_out_masks = torch.zeros((16,1,img_dim,img_dim)).cuda()
    display_in_masks = torch.zeros((16,1,img_dim,img_dim)).cuda()
    cnt_display = 0
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        #true_mask_cv = true_mask.cpu().detach().numpy()
        ...
        loss_b, loss_t, recon_out, embeddings_lo, embeddings_hi = model(in_image)
        recon_image_general = recon_out
        recon_gen_depth = recon_image_general[:, :1, :, :]
        recon_gen_rgb = recon_image_general[:, 1:, :, :]

        recon_feat_hi, recon_embeddings_hi, loss_b = sub_res_model_hi(embeddings_hi, embedder_hi)
        recon_feat_lo, recon_embeddings_lo, loss_b_lo = sub_res_model_lo(embeddings_lo, embedder_lo)
        ...
        recon_depth = recon_image_recon[:, :1, :, :]
        recon_rgb = recon_image_recon[:, 1:, :, :]
        ...
        if i_batch in display_indices:
            ...
            display_images[cnt_display] = recon_depth[0]
            display_rgb_images[cnt_display] = recon_rgb[0]
            display_gt_images[cnt_display] = depth_image[0]
            display_gt_images_rgb[cnt_display] = rgb_image[0]
    ...
    for obj_name in obj_names:
        ...
        for epoch in range(epochs):
            for i_batch, sample_batched in enumerate(dataloader):
                ...
                segment_loss = segment_loss + l1_mask_loss # L1 is different than in the paper but may improve results in some cases
