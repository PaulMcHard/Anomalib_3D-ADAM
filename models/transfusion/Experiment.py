...
def train(args):
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=4, shuffle=False, num_workers=8
    )
...
def evaluate(PATH, testset, net, parameters, load_weights=False, category_name=None):
    ...
    all_masks = np.zeros((len(testset), img_width, img_height))
    true_masks = np.zeros((len(testset), img_width, img_height))
