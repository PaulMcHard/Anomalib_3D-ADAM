...
def get_backgrounds(imgs, out_path):
    ...
    for idx, img in enumerate(imgs):
        ...
        img_file = cv2.resize(img_file, (320,256))
        
        with torch.no_grad():
            sam.set_image(img_file)
            input_points = [[0,0],[319,0],[0,255],[319,255]]
            final_mask = np.zeros((1, 320,256))
            ...
            print(idx)
            cv2.imwrite(f"{out_path}/{idx}", final_mask.transpose((2,1,0)))
...
def get_masks(args):
    ...
    for category in categories:
        ...
        print(f"{final_path}*/images/*.{args.file_ending}")
        imgs = glob.glob(f"{final_path}*/images/*.{args.file_ending}")
        print(imgs)
