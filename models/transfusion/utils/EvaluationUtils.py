...
def calculate_transfussion_results(net, loader, index):
    ...
    img_height = true.shape[-1]
    img_width = true.shape[-2]
    ...
    if true.shape[0] > 4:
        true = true[:4, :, :]  # Keep only the first 4 channels

    true = normalize(true)
    x = normalize(x)

    mask_disc = np.expand_dims(mask, 0)
    mask_recon = np.zeros((img_width, img_height))
    #print(f"mask_recon: {mask_recon.shape}, true: {true.shape}, x: {x.shape}")
